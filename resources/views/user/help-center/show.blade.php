...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        ...
        <a href="{{ route('user-help-center.index') }}" class="btn btn-outline-secondary btn-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex flex-column gap-3 mb-4" style="max-height: 420px; overflow-y: auto;">
                        @forelse ($helpCenter->answers as $answer)
                            @php $isMe = $answer->user_id === $helpCenter->user_id; @endphp
                            <div class="d-flex {{ $isMe ? 'justify-content-end' : 'justify-content-start' }}">
                                <div class="p-2 px-3 rounded-3 {{ $isMe ? 'bg-primary-subtle' : 'bg-light' }}" style="max-width: 75%;">
                                    <div class="fs-12 fw-semibold {{ $isMe ? 'text-primary' : 'text-muted' }} mb-1">
                                        @if ($isMe)
                                            Anda
                                        @else
                                            {{ $answer->user->name ?? 'Admin' }} (Admin)
                                        @endif
                                    </div>
                    ...
                    <form action="{{ route('user-help-center.reply', $helpCenter->id) }}" method="POST">
                        @csrf
                        <div class="mb-2">

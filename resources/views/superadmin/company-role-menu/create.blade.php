...
@section('content')
    <div class="row justify-content-center">
        <div class="col-lg-7">
            <div class="card">
                <div class="card-body">
                    <h4 class="mb-4">Tambah Company Role Menu</h4>
                    ...
                    <form action="{{ route('company-role-menu.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select name="category_application_id" id="category_application_id" class="form-select" required>
                                ...
                                @foreach ($categoryApplications as $category)
                                    <option value="{{ $category->id }}" @selected(old('category_application_id') == $category->id)>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="application_menu_id" id="application_menu_id" class="form-select" required>
                                ...
                                @foreach ($applicationMenus as $menu)
                                    <option value="{{ $menu->id }}" data-category="{{ $menu->category_application_id }}"
                                        @selected(old('application_menu_id') == $menu->id)>
                                        {{ $menu->name }}
                                    </option>
                                @endforeach
                            </select>

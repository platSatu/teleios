 ...
 * tryJadwalConfirmation() below): checked before keyword rules or the
 * AI bot, and short-circuits the rest of this method when it fires — a
 * jadwal reminder reply is always more specific/contextual than a
 * company's generic auto-reply keywords, so it should never be shadowed
 * by e.g. a keyword rule that also happens to match "ya".
 *
 * Checked right after that, still before the keyword-rule chain: Fitur
 * #6's multi-step chatbot flows (see tryChatbotFlow() below /
 * App\Services\Chat\ChatbotFlowService) — a customer already mid-flow
 * has their reply routed through the flow engine instead of the ordinary
 * keyword chain, and a message that happens to match a flow's own
 * trigger_keyword starts one. Only falls through to the plain keyword
 * chain when the message has nothing to do with any flow at all.
 *
...
class WaIncomingMessageWebhookController extends Controller
{
    ...
    /**
     * Same "whole-message, case-insensitive" match as CONFIRM_WORDS/
     * DECLINE_WORDS above — deliberately not a substring match, so an
     * unrelated longer message that merely contains "stop" somewhere
     * (e.g. "tolong stop dulu jadwalnya ya kak") doesn't accidentally
     * unsubscribe someone. Indonesian and English both included since
     * this app's audience is Indonesian SMEs but "STOP" is the
     * globally-recognized opt-out word WhatsApp itself expects
     * businesses to honor.
     */
    ...
    public function handle(Request $request): JsonResponse
    {
        ...
        // (Jadwal confirmation, keyword rules, the AI bot): a customer
        // saying STOP is a compliance/anti-ban signal, not a
        // conversational one, so it must never be shadowed by a
        // company's own auto-reply keyword that happens to also match.
        // Short-circuits the rest of this method when it fires, same
        // pattern as tryJadwalConfirmation() below.
        ...
        // auto-reply/AI-bot/Jadwal-confirmation logic below, which is
        // what this webhook exists for in the first place.
        try {
    ...
    /**
     * Whole-message match against OPT_OUT_WORDS/OPT_IN_WORDS (same
     * matching style as parseConfirmationIntent() below) — on a match,
     * records the opt-out/opt-in via App\Services\Chat\
     * BroadcastOptOutService and queues a short acknowledgement reply,
     * then short-circuits the rest of handle() so this never also
     * triggers a company's own keyword auto-reply.
     *

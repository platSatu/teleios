...
Route::prefix('dashboard')->middleware(['auth', 'verified'])->group(function () {
    ...
    // "Jadwal" — new top-level sidebar group, sibling to "Chat" (not
    // nested under it), and its own SEPARATE package — a user with an
    // active Chat package does NOT automatically get Jadwal, and vice
    // versa. 'active.package:Jadwal' (see App\Http\Middleware\
    // EnsureActivePackage's category-filter argument) only counts a
    // voucher whose package belongs to the "Jadwal" category_applications
    // row (seeded by 2026_08_07_120000_seed_jadwal_kelas_application_menu),
    // never just any active package. See App\Http\Controllers\Jadwal\*
    // and resources/views/jadwal/*.
    Route::prefix('jadwal')->middleware(['active.package:Jadwal', 'menu.access'])->group(function () {
        Route::prefix('mata-pelajaran')
            ->controller(\App\Http\Controllers\Jadwal\MataPelajaranController::class)
            ->group(function () {
        ...
        Route::prefix('jadwal-kelas')
            ->controller(\App\Http\Controllers\Jadwal\JadwalKelasController::class)
            ->group(function () {
                Route::get('/', 'index')->name('jadwal.jadwal-kelas.index');
                Route::get('/create', 'create')->name('jadwal.jadwal-kelas.create');
                Route::post('/', 'store')->name('jadwal.jadwal-kelas.store');
                Route::get('/{id}', 'show')->name('jadwal.jadwal-kelas.show');
                Route::get('/{id}/edit', 'edit')->name('jadwal.jadwal-kelas.edit');
                Route::put('/{id}', 'update')->name('jadwal.jadwal-kelas.update');
                Route::delete('/{id}', 'destroy')->name('jadwal.jadwal-kelas.destroy');
            });

        // Nested under one jadwal-kelas — roster (murid) + generated
        // dated occurrences (sesi) only ever make sense in the context
        // of one specific class, so neither gets its own top-level
        // index (see both controllers' docblocks).
        Route::prefix('jadwal-kelas/{jadwalKelas}')
            ->group(function () {
                Route::controller(\App\Http\Controllers\Jadwal\JadwalKelasMuridController::class)
                    ->prefix('murid')
                    ->group(function () {
                        Route::post('/', 'store')->name('jadwal.jadwal-kelas.murid.store');
                        Route::delete('/{id}', 'destroy')->name('jadwal.jadwal-kelas.murid.destroy');
                    });

                Route::post('/sesi/generate', [\App\Http\Controllers\Jadwal\JadwalKelasSesiController::class, 'generate'])
                    ->name('jadwal.jadwal-kelas.sesi.generate');
            });

        Route::put('/sesi-murid/{id}/status', [\App\Http\Controllers\Jadwal\JadwalKelasSesiController::class, 'updateStatus'])
            ->name('jadwal.jadwal-kelas.sesi-murid.update-status');

        // "Jadwal khusus pengajar" — clicking a guru's name (Jadwal
        // Kelas index / anywhere else in the module) lands here: every
        // class they teach + upcoming sesi + the quick actions below.
        Route::get('/guru/{guruUserId}', [\App\Http\Controllers\Jadwal\JadwalGuruController::class, 'show'])
            ->name('jadwal.jadwal-kelas.guru.show');

        // Custom WA wording per company for the highest-frequency Jadwal
        // messages (H-1 reminders + reply acks) — see Jadwal\
        // JadwalMessageTemplateController's own docblock.
        Route::prefix('pengaturan-pesan')
            ->controller(\App\Http\Controllers\Jadwal\JadwalMessageTemplateController::class)
            ->group(function () {
                Route::get('/', 'index')->name('jadwal.pengaturan-pesan.index');
                Route::put('/', 'update')->name('jadwal.pengaturan-pesan.update');
                Route::delete('/{key}', 'reset')->name('jadwal.pengaturan-pesan.reset');
            });

        // One-off per-date time change ("guru memajukan jam mengajar").
        Route::put('/sesi/{id}/reschedule-time', [\App\Http\Controllers\Jadwal\JadwalKelasSesiController::class, 'rescheduleTime'])
            ->name('jadwal.jadwal-kelas.sesi.reschedule-time');

        // Guru sakit -> cari & tetapkan pengganti untuk satu sesi.
        Route::post('/sesi/{id}/sakit', [\App\Http\Controllers\Jadwal\JadwalKelasSesiController::class, 'markSakit'])
            ->name('jadwal.jadwal-kelas.sesi.mark-sakit');
        Route::get('/sesi/{id}/pengganti', [\App\Http\Controllers\Jadwal\JadwalKelasSesiController::class, 'penggantiForm'])
            ->name('jadwal.jadwal-kelas.sesi.pengganti');
        Route::post('/sesi/{id}/pengganti', [\App\Http\Controllers\Jadwal\JadwalKelasSesiController::class, 'assignPengganti'])
            ->name('jadwal.jadwal-kelas.sesi.assign-pengganti');

        // Murid mau geser jadwal -> sistem cari & tawarkan slot alternatif.
        Route::get('/sesi-murid/{id}/alternatif', [\App\Http\Controllers\Jadwal\JadwalKelasSesiController::class, 'alternatifMurid'])
            ->name('jadwal.jadwal-kelas.sesi-murid.alternatif');
        Route::post('/sesi-murid/{id}/pindah', [\App\Http\Controllers\Jadwal\JadwalKelasSesiController::class, 'requestPindah'])
            ->name('jadwal.jadwal-kelas.sesi-murid.pindah');

        // Custom makeup date/time with the SAME guru, pending their WA
        // approval — see Jadwal\JadwalUsulanController's own docblock
        // for how this differs from the "pindah ke kelas lain" flow
        // right above.
        Route::post('/sesi-murid/{id}/usulan', [\App\Http\Controllers\Jadwal\JadwalUsulanController::class, 'store'])
            ->name('jadwal.jadwal-kelas.sesi-murid.usulan');
    });
});
